...
// setupFailedDetailTest 创建返回 400 的上游与 DeepSeek 别名渠道，返回捕获的
// 出站请求体与错误响应体。
func setupFailedDetailTest(t *testing.T, upstreamBody string, status int) (*httptest.Server, *[]byte) {
	var captured []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
...
func createFailedDetailChannelGroup(t *testing.T, ctx context.Context, name string, server *httptest.Server) {
	channel := &model.Channel{
		Name:                  name,
		Type:                  outbound.OutboundTypeOpenAIChat,
		Enabled:               true,
		...
		ForceDeepSeekThinking: true,
		Keys:                  []model.ChannelKey{{Enabled: true, ChannelKey: "test-key"}},
	}
	if err := op.ChannelCreate(channel, ctx); err != nil {
		t.Fatalf("ChannelCreate failed: %v", err)
	}
	group := &model.Group{Name: name + "-group", Mode: model.GroupModeFailover}
	if err := op.GroupCreate(group, ctx); err != nil {
		t.Fatalf("GroupCreate failed: %v", err)
	}
	if err := op.GroupItemAdd(&model.GroupItem{GroupID: group.ID, ChannelID: channel.ID, ModelName: "deepseek-v4-flash", Priority: 1, Weight: 1}, ctx); err != nil {
		t.Fatalf("GroupItemAdd failed: %v", err)
	}
}

func runFailedDetailRequest(t *testing.T, groupName string) {
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Set("api_key_id", 10)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages",
		strings.NewReader(`{"model":"`+groupName+`","max_tokens":128,"thinking":{"type":"enabled","budget_tokens":4096},"messages":[{"role":"user","content":"hello"}]}`))
	c.Request.Header.Set("Content-Type", "application/json")
	Handler(inbound.InboundTypeAnthropic, c)
}

// TestHandlerFailedAttemptRecordsRequestResponseBody verifies failed attempts
// persist the outbound request body (post-transform upstream shape) and the
// failure response body, gated by the relay_log_failed_detail_enabled setting.
func TestHandlerFailedAttemptRecordsRequestResponseBody(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ctx := setupRelayTestDB(t)

	server, captured := setupFailedDetailTest(t, `{"error":{"message":"upstream boom"}}`, http.StatusBadRequest)
	defer server.Close()
	createFailedDetailChannelGroup(t, ctx, "relay-failed-detail", server)
	runFailedDetailRequest(t, "relay-failed-detail-group")

	logs, err := op.RelayLogList(ctx, nil, nil, nil, 1, 10)
	if err != nil {
		t.Fatalf("RelayLogList failed: %v", err)
	}
	var found *model.RelayLog
	for i := range logs {
		if logs[i].ChannelName == "relay-failed-detail" {
			found = &logs[i]
			break
		}
	}
	if found == nil {
		t.Fatalf("failed relay log not found: %+v", logs)
	}
	if len(found.Attempts) == 0 {
		t.Fatalf("expected attempts recorded")
	}
	attempt := found.Attempts[0]
	if attempt.RequestBody == "" {
		t.Fatalf("expected request_body recorded for failed attempt")
	}
	if !strings.Contains(attempt.RequestBody, `"model":"deepseek-v4-flash"`) {
		t.Fatalf("request_body should be the outbound (post-transform) payload, got: %s", attempt.RequestBody)
	}
...
// TestHandlerFailedDetailDisabledSkipsRecording verifies the switch disables
// failed-attempt detail capture entirely.
func TestHandlerFailedDetailDisabledSkipsRecording(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ctx := setupRelayTestDB(t)
	if err := op.SettingSetString(model.SettingKeyRelayLogFailedDetailEnabled, "false"); err != nil {
		t.Fatalf("SettingSetString failed: %v", err)
	}

	server, _ := setupFailedDetailTest(t, `{"error":{"message":"boom"}}`, http.StatusBadRequest)
	defer server.Close()
	createFailedDetailChannelGroup(t, ctx, "relay-failed-detail-off", server)
	runFailedDetailRequest(t, "relay-failed-detail-off-group")
	...
	var found *model.RelayLog
	for i := range logs {
		if logs[i].ChannelName == "relay-failed-detail-off" {
			found = &logs[i]
			break
		}
	}
	if found == nil {
		t.Fatalf("failed relay log not found")
	}
	if len(found.Attempts) == 0 {
		t.Fatalf("expected attempts recorded")
	}
	if found.Attempts[0].RequestBody != "" || found.Attempts[0].ResponseBody != "" {
		t.Fatalf("expected no failed details when switch disabled, got req=%q resp=%q",
			found.Attempts[0].RequestBody, found.Attempts[0].ResponseBody)
	}
}

// TestHandlerFailedDetailTruncated verifies failed details are capped at
// maxFailedDetailBytes.
func TestHandlerFailedDetailTruncated(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ctx := setupRelayTestDB(t)

	bigBody := `{"error":{"message":"` + strings.Repeat("x", 200*1024) + `"}}`
	server, _ := setupFailedDetailTest(t, bigBody, http.StatusBadRequest)
	defer server.Close()
	createFailedDetailChannelGroup(t, ctx, "relay-failed-detail-trunc", server)
	runFailedDetailRequest(t, "relay-failed-detail-trunc-group")
	...
	var found *model.RelayLog
	for i := range logs {
		if logs[i].ChannelName == "relay-failed-detail-trunc" {
			found = &logs[i]
			break
		}
	}
	if found == nil || len(found.Attempts) == 0 {
		t.Fatalf("failed relay log or attempts missing")
	}
	if len(found.Attempts[0].ResponseBody) > maxFailedDetailBytes {
