...
- **渠道标签**：渠道新增 `tags` 标签列表，渠道卡片与编辑表单均展示/编辑，支持按标签搜索渠道。
- **API Key 标签白名单**：API Key 设置中可选择标签。选择后 `/v1/models` 只返回带有指定标签渠道自身暴露的模型，调用也只能路由到这些渠道的模型；标签受限时绕过已有分组，不把分组同名模型纳入请求。
- **限制组合**：标签白名单与渠道 ID 白名单、模型白名单同时配置时取交集。

### 渠道模型重定向

- **重定向配置**：渠道编辑中可设置「别名模型 → 上游实际模型」，请求别名时自动改写为上游模型名。
- **仅暴露别名**：开启后渠道原始 Model/CustomModel 不再出现在 `/v1/models`、渠道模型列表与分组路由中，仅暴露重定向别名模型。
- **协议覆盖**：文本（chat/messages/responses/embeddings）、responses/compact、图片与 WebSocket 路由均已接入重定向解析。
